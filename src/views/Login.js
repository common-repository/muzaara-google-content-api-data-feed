...
const ListAccounts = ({accounts, selectAccount, selected, isLinking, linkError, link}) => {
    if ( accounts.length == 0 ) {
        return <span>{MUZAARA_WOOPF.l10n.no_account_found}</span>
    } 
    ...
    return(
        <div>
            <fieldset>
                <legend style={{ fontSize: "1.3em", marginBottom: "10px"}}>{MUZAARA_WOOPF.l10n.chooseAccount}</legend>
                {accountFields}
                <p>
...
export const GoogleLogin = ({authenticating, onClick, accounts, selectAccount, selected, isLinking, linkError, link}) => {
    return (
        ...
        <div className="card">
            <h2 className="title">
                {MUZAARA_WOOPF.l10n.linkGoogleDesc}
            </h2>
            <p>
                {MUZAARA_WOOPF.l10n.linkGoogleDesc}
            </p>
                    ...
                    (
                         accounts == null ? (
                            <div className="spinner is-active" style={{
                                ...
                                {MUZAARA_WOOPF.l10n.linkingAccount}
                            </div>
                        ) : (
