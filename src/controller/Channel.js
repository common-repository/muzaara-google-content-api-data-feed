...
class Channel extends React.Component {
    ...
    getChannel() {
        if ( this.state.channel.id ) {
            this.setState({ gettingChannel: true}, () => {
                request({
                    method: "POST",
                    body: { action: "muzaara_woopf_getFeed", feed_id: this.state.channel.id }
                })
    ...
    getProductTypes() {
        this.setState({creatingLoading: true}, () => {
            request({
                method: "POST",
                body: {action: "muzaara_woopf_getProductTypes"}
            })
            .then( response => response.json() )
            .then( json => {
    ...
    getConditions() {
        request({
            method: "POST",
            body: {action: "muzaara_woopf_getConditions" }
        })
        .then( response => response.json() )
        .then( json => {
    ...
    createChannel( e ) {
        ...
        this.setState({creating: true, errors: []}, () => {
            request({
                method: "POST",
                body: Object.assign({action: "muzaara_woopf_createChannel"}, this.state.channel )
            })
            .then( response => response.json() )
            .then( json => {
                if ( json.success ) {
        ...
        if ( channel.categoryMappings[id].trim().length >= 3 || !isNaN(channel.categoryMappings[id] ) ) {
            ...
            this.searchInt = setTimeout(() => {
                this.setState({ gcatSearch: {0: getText( "searching" )}, activeCatMap: id}, () => {
                    request({
                        method: "POST", 
                        body: {action: "muzaara_woopf_searchGoogleCat", gcat_q: channel.categoryMappings[id].trim()}
                    })
                    .then( response => response.json() )
                    .then( json => {
                        let gcatSearch = {}
    ...
    getFields(t) {
        this.setState({creatingLoading: true}, () => {
            request({
                method: "POST",
                body: {action: "muzaara_woopf_getProductFields"}
            })
            .then( response => response.json() )
            .then( json => {
    ...
    getCategories() {
        request({
            method: "POST",
            body: {action: "muzaara_woopf_getProductCategories"}
        })
        .then(response => response.json() )
        .then( json => {
            let channel = Object.assign({}, this.state.channel)
