...
class Home extends React.Component {
    constructor(props) {
        ...
        this.state = {
            isReady: parseInt(MUZAARA_WOOPF.hasAccess),
            channels: [],
    ...
    checkReady() {
        return request({
            method: "POST",
            body: { action: "muzaara_woopf_checkLink" }
        })
        ...
        if ( index < this.state.channels.length ) {
            if ( confirm( getText("deleteConfirmation" ).replace( "%s", this.state.channels[index]["name"] ) ) ) {
                ...
                this.setState({channels}, () => {
                    request({
                        method: "POST",
                        body: {action: "muzaara_woopf_deleteFeed", feed_id: id}
                    })
                })
            }
        }
    ...
    pauseChannel(index,e ) {
        ...
        if ( index < this.state.channels.length ) {
            ...
            this.setState({channels}, () => {
                request({
                    method: "POST",
                    body: {action: ( !this.state.channels[index].is_active ? "muzaara_woopf_pauseFeed" : "muzaara_woopf_resumeFeed" ), feed_id: channels[index]["id"] }
                })
            })
        }
    }
    ...
    runChannel(index, e) {
        ...
        if ( index < this.state.channels.length ) {
            ...
            this.setState({channels}, () => {
                request({
                    method: "POST",
                    body: { action: "muzaara_woopf_runFeed", feed_id: channels[index][ "id" ] }
                })
                .then( response => response.json() )
                .then( json => {
                    let channels = [...this.state.channels]
    ...
    getChannels() {
        this.setState({gettingChannels: true}, () => {
            request({
                method: "POST",
                body: {action: "muzaara_woopf_getFeeds"}
            })
            .then( response => response.json() )
            .then( json => {
                if ( json.success ) {
