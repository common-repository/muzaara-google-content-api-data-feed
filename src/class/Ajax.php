...
use function \Muzaara\Engine\Functions\Google\Ads\getAccessibleAccounts;
use function \Muzaara\Engine\Functions\Google\Ads\sendAcceptInvitation;
use function \Muzaara\ProductFeed\Helpers\createFeed;
use function Muzaara\ProductFeed\Helpers\getFeeds;
use function Muzaara\ProductFeed\Helpers\pauseFeed;
use function Muzaara\ProductFeed\Helpers\deleteFeed;
use function Muzaara\ProductFeed\Helpers\resumeFeed;
use function Muzaara\ProductFeed\Helpers\getFeed;
...
class Ajax {
    ...
    private function actions() {
        add_action( "wp_ajax_muzaara_woopf_checkAuth", array($this, "checkAuth" ) );
        add_action( "wp_ajax_muzaara_woopf_getAccounts", array($this, "getAccounts" ) );
        add_action( "wp_ajax_muzaara_woopf_linkAccount", array($this, "linkAccount" ) );
        add_action( "wp_ajax_muzaara_woopf_checkLink", array($this, "checkLink" ) );
        add_action( "wp_ajax_muzaara_woopf_getProductFields", array($this, "getProductFields" ) );
        add_action( "wp_ajax_muzaara_woopf_getProductCategories", array($this, "getProductCategories" ) );
        add_action( "wp_ajax_muzaara_woopf_searchGoogleCat", array($this, "searchGoogleCat" ) );
        add_action( "wp_ajax_muzaara_woopf_getConditions", array( $this, "getConditions" ) );
        add_action( "wp_ajax_muzaara_woopf_createChannel", array($this, "createChannel" ) );
        add_action( "wp_ajax_muzaara_woopf_getProductTypes", array($this, "getProductTypes" ) );
        add_action( "wp_ajax_muzaara_woopf_getFeeds", array( $this, "getFeeds" ) );
        add_action( "wp_ajax_muzaara_woopf_pauseFeed", array( $this, "pauseFeed" ) );
        add_action( "wp_ajax_muzaara_woopf_deleteFeed", array( $this, "deleteFeed" ) );
        add_action( "wp_ajax_muzaara_woopf_resumeFeed", array( $this, "resumeFeed" ) );
        add_action( "wp_ajax_muzaara_woopf_getFeed", array( $this, "getFeed" ) );
        add_action( "wp_ajax_muzaara_woopf_runFeed", array( $this, "runFeed" ) );
    }
    ...
    public function getAccounts() {
        if ( current_user_can( $this->app->cap ) ) {
            try {
                ...
                wp_send_json_error( __( "No Ads account found", "muzaara-woopf" ) );
            } catch ( \Exception $e ) {
    ...
    public function linkAccount() {
        if ( current_user_can($this->app->cap) && !empty( $_POST[ "account_id" ] ) ) {
            ...
            wp_send_json_error(array("data" => __( "Unable to connect to API Server. Please contact plugin support", "muzaara-woopf" ) ) );
        }
    ...
    public function getProductFields() {
        if ( current_user_can($this->app->cap) ) {
            $channels = $this->app->getInstance( "Muzaara\ProductFeed\Channels" );
    ...
    public function getConditions() {
        if ( current_user_can($this->app->cap) ) {
            $channelsInstance = $this->app->getInstance( "Muzaara\ProductFeed\Channels" );
