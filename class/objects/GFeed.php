...
use function Muzaara\ProductFeed\Helpers\filterInit;
use function Muzaara\ProductFeed\Helpers\fieldInit;
use function Muzaara\ProductFeed\Helpers\ruleInit;

class GFeed extends \Muzaara\ProductFeed\Abs\Feed {
    ...
    public function getMerchantId() {
        return apply_filters( "muzaara_woopf_get_feed_merchant_id", $this->merchantId, $this );
    }

    public function getNoticeEmail() {
        return apply_filters( "muzaara_woopf_get_feed_notice_email", (!$this->noticeEmail ? get_bloginfo( "admin_email" ) : $this->noticeEmail), $this );
    }
    ...
    public function getUtm( $product = null ) {
        ...
        return apply_filters( "muzaara_woopf_get_feed_utm", $utm, $this );
    }
    ...
    public function getFilters() : array {
        return apply_filters( "muzaara_woopf_get_feed_filters", $this->filters, $this );
    }

    public function getRules() : array {
        return apply_filters( "muzaara_woopf_get_feed_rules", $this->rules, $this );
    }
    ...
    public function toArgs() {
        $postargs = array(
            ...
            "post_type"         =>  MUZAARA_WOOPF_POST_TYPE,
            "meta_input"        =>  array(
                "muzaara_woopf_push_type"           =>  $this->pushType,
                "muzaara_woopf_refresh_rate"        =>  $this->refreshRate,
                "muzaara_woopf_country"             =>  $this->country,
                "muzaara_woopf_category_mapping"    =>  $this->categoryMapping,
                "muzaara_woopf_mappings"            =>  array(),
                "muzaara_woopf_filters"             =>  array(),
                "muzaara_woopf_rules"               =>  array(),
                "muzaara_woopf_utm"                 =>  $this->utm,
                "muzaara_woopf_feed_type"           =>  $this->type,
                "muzaara_woopf_merchant_id"         =>  $this->merchantId,
                "muzaara_woopf_notice_email"        =>  $this->noticeEmail,
                "muzaara_woopf_running_status"      =>  $this->runningStatus === true,
                "muzaara_woopf_product_ids"         =>  $this->productIds
            )
        );

        
        if ( !$this->id || ( $this->id && $this->lastRefreshed ) ) {
            $postargs[ "meta_input" ][ "muzaara_woopf_last_refreshed" ] = $this->lastRefreshed;
        }

        if ( $this->dumpURL ) {
            $postargs[ "meta_input" ][ "muzaara_woopf_dump_url" ] = $this->dumpURL;
        }
        ...
        if ( $this->mappings ) {
            ...
            $postargs[ "meta_input" ][ "muzaara_woopf_mappings" ] = $mappings;
        }
        ...
        if ( $this->filters ) {
            ...
            $postargs[ "meta_input" ][ "muzaara_woopf_filters" ] = $filters;
        }
        ...
        if ( $this->rules ) {
            ...
            $postargs[ "meta_input" ][ "muzaara_woopf_rules" ] = $rules;
        }

        return apply_filters( "muzaara_woopf_feed_to_postargs", $postargs, $this );
    }
    ...
    public function save( $push_products = false ) {
        ...
        do_action( "muzaara_woopf_before_feed_create", $this );
        if ( !$this->id ) {
        ...
        if ( !is_wp_error( $post_id ) && $post_id ) {
            ...
            */
            ...
            delete_post_meta( $post_id, "muzaara_woopf_product_types" ); // delete and add again to prevent duplicates

            foreach( $this->productTypes as $productType ) {
                add_post_meta($post_id, "muzaara_woopf_product_types", $productType, false);
            }
        ...
        do_action( "muzaara_woopf_after_feed_create", $this );
    ...
    public function findProducts( $exclude_already_pushed = true ) : array {
        ...
        return apply_filters( "muzaara_woopf_find_feed_products", $products, $this );
    }
    ...
    public function generateDump() {
        ...
        if ( !file_exists( MUZAARA_WOOPF_DUMP_PATH ) ) {
            mkdir(MUZAARA_WOOPF_DUMP_PATH);
        }

        $filepath = sprintf( "%s%s", MUZAARA_WOOPF_DUMP_PATH, $filename );
        ...
        $this->setDumpURL( sprintf( "%s%s", MUZAARA_WOOPF_DUMP_URL, $filename ) );
    ...
    public function pushBatchProducts() {
        ...
        try {
            $session = new \Google_Service_ShoppingContent( \Muzaara\Engine\Functions\Google\getClient() );
            ...
            foreach( $batchResponse->entries as $index => $entry ) {
                if ( !empty( $entry->getErrors() ) ) {
                    ...
                    $products[$index]->update_meta_data( "muzaara_woopf_content_id", sanitize_text_field( $entry->product->getId() ) );
            ...
            wp_mail( $noticeEmail, sprintf( __( "Error pushing Bulk Product using Content API", "muzaara-woopf" ) ), $e->getMessage() );
        }
    }
    ...
    public function pushProduct( $product_id ) {
        ...
        $existingId = $product->get_meta( "muzaara_woopf_content_id", true );

        if ( $this->getPushType() == 1 && $product && $merchantId ) {
                if ( $product->is_type( 'variable' ) ) {
                        $variations = $product->get_available_variations( 'objects' );
                        foreach ( $variations as $variation ) {
                                $this->pushProduct( $variation->get_id() );
                        }
                } else {
                        try {
                                $session = new \Google_Service_ShoppingContent( \Muzaara\Engine\Functions\Google\getClient() );
                                ...
                                $product->update_meta_data( "muzaara_woopf_content_id", sanitize_text_field( $newProduct->getId() ) );
                                ...
                                wp_mail( $noticeEmail, sprintf( __( "Error pushing Product %s using Content API", "muzaara-woopf" ), $product->get_name() ), $e->getMessage() );
