...
abstract class Condition {
    ...
    // abstract public function setStmt(\Muzaara\ProductFeed\Object\Field $fieldA, \Muzaara\ProductFeed\Object\Field $fieldB, int $action);
    ...
    public function getName() : string {
        ...
        switch($this->condition) {
            case self::CONDITION_CONTAINS:
                $ret = __( "CONTAINS (LIKE)", "muzaara-woopf" );
            break;
            case self::CONDITION_NOT_CONTAINS:
                $ret = __( "NOT CONTAINS (NOT LIKE)", "muzaara-woopf" );
            break;
            case self::CONDITION_EQUALS:
                $ret = __( "EQUALS (=)", "muzaara-woopf" );
            break;
            case self::CONDITION_NOT_EQUALS:
                $ret = __( "NOT EQUALS (!=)", "muzaara-woopf" );
            break;
            case self::CONDITION_GREATER_THAN:
                $ret = __( "GREATER THAN (>)", "muzaara-woopf" );
            break;
            case self::CONDITION_GREATER_EQUALS:
                $ret = __( "GREATER THAN & EQUALS (>=)", "muzaara-woopf" );
            break;
            case self::CONDITION_LESS_THAN:
                $ret = __( "LESS THAN (<)", "muzaara-woopf" );
            break;
            case self::CONDITION_LESS_EQUALS:
                $ret = __( "LESS THAN EQUALS (<=)", "muzaara-woopf" );
            break;
            case self::CONDITION_IS_EMPTY:
                $ret = __( "IS EMPTY (!)", "muzaara-woopf" );
            break;
            case self::CONDITION_IS_NOT_EMPTY:
                $ret = __( "IS NOT EMPTY (!!)", "muzaara-woopf" );
            break;
            case self::CONDITION_IS_IN:
                $ret = __( "IN (..,..,)", "muzaara-woopf" );
            break;
            case self::CONDITION_IS_NOT_IN:
                $ret = __( "NOT IN !(..,..,)", "muzaara-woopf" );
            break;
            case self::CONDITION_BETWEEN:
                $ret = __( "BETWEEN (...)", "muzaara-woopf" );
            break;
            case self::CONDITION_NOT_BETWEEN:
                $ret = __( "NOT BETWEEN !(...)", "muzaara-woopf" );
            break;
            default:
                $ret = "";
        }

        return apply_filters( "muzaara_woopf_get_condition_name", $ret, $this );
    }

    // public function getId() {
    //     return apply_filters( "muzaara_woopf_get_condition_id", $this->id, $this );
    // }

    public function getCondition() {
        return apply_filters( "muzaara_woopf_get_condition_condition", $this->condition, $this );
    }
}