...
require_once MUZAARA_WOOPF_OBJ_PATH . "GFeed.php";

use Muzaara\ProductFeed\Object\GFeed;
...
if ( !function_exists( "createFeed" ) ) {
    function createFeed(array $postargs) {
        ...
        return new \WP_Error( "no_title", __( "Unable to create feed: No title or product types", "muzaara-woopf" ));
    }
}
...
function getFeed(int $postId) {
    ...
    if ( $post && $post->post_type == MUZAARA_WOOPF_POST_TYPE ) {
        $feedType = get_post_meta($postId, "muzaara_woopf_feed_type", true);
        switch( $feedType ) {
            case "google":
        ...
        if ( $feed ) {
            $feed->setName($post->post_title)
                ->setPushType( get_post_meta($postId, "muzaara_woopf_push_type", true) )
                ->setRefreshRate( get_post_meta($postId, "muzaara_woopf_refresh_rate", true) )
                ->setProductTypes(get_post_meta( $postId, "muzaara_woopf_product_types" ))
                ->setStatus($post->post_status)
                ->setLastRefreshed( get_post_meta( $postId, "muzaara_woopf_last_refreshed", true ) )
                ->setDumpURL( get_post_meta( $postId, "muzaara_woopf_dump_url", true ) )
                ->setMerchantId( get_post_meta( $postId, "muzaara_woopf_merchant_id", true ) )
                ->setRunningStatus( (bool) get_post_meta( $postId, "muzaara_woopf_running_status", true ) )
                ->setCountry( get_post_meta( $postId, "muzaara_woopf_country", true ) );

            $feed->setNoticeEmail( get_post_meta( $postId, "muzaara_woopf_notice_email", true ) );
            $categories = get_post_meta( $postId, "muzaara_woopf_category_mapping", true);
            $mappings = get_post_meta( $postId, "muzaara_woopf_mappings", true );
            $filters = get_post_meta( $postId, "muzaara_woopf_filters", true );
            $rules = get_post_meta( $postId, "muzaara_woopf_rules", true );
            $utm = get_post_meta( $postId, "muzaara_woopf_utm", true );
            $lastRefreshed = get_post_meta( $postId, "muzaara_woopf_last_refreshed", true );
            $productIds = get_post_meta( $postId, "muzaara_woopf_product_ids", true );
...
function pauseFeed($feed) {
    ...
    do_action( "muzaara_woopf_feed_paused", $feed );
...
function resumeFeed( $feed ) {
    ...
    do_action( "muzaara_woopf_feed_resumed", $feed );
...
function findFeedMatch( $product_id, $exact = false ) {
    ...
    if ( $product ) {
        $postargs = array(
            "post_status"   =>  "publish",
            "post_type"     =>  MUZAARA_WOOPF_POST_TYPE,
            "meta_key"      =>  "muzaara_woopf_product_types",
